  ...
  return <div className="flex h-[calc(100vh-2rem)] min-h-0 w-full min-w-0 flex-col overflow-hidden border border-slate-300 bg-white shadow-sm">
    ...
    <nav className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-5 py-2" aria-label="Timetable steps">
      ...
      <div className="ml-auto flex items-center gap-2">
        {data.rooms.length === 0 && <span className="text-[11px] text-amber-700">Rooms must be added before generation</span>}
        {!showRules && <button type="button" title={workloadDifference !== 0 ? `Set exactly ${weeklyCapacity} course periods before generating.` : 'Automatically fill the week, prioritizing higher-credit courses and keeping required blocks together.'} disabled={busy || data.rooms.length === 0 || offerings.length === 0 || weeklyCapacity === 0 || workloadDifference !== 0} onClick={() => void applyRulesAndGenerate()} className="inline-flex h-9 items-center gap-2 rounded-lg bg-violet-700 px-4 text-xs font-semibold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40"><Sparkles size={14} /> Generate timetable</button>}
        {!showRules && entries.length !== weeklyCapacity && <span className="text-[11px] text-amber-700">Generate all {weeklyCapacity} periods for this class before publishing</span>}
        {!showRules && <button type="button" title={selectedVersion?.status !== 'draft' ? 'Choose Edit timetable to create a working copy first.' : entries.length !== weeklyCapacity ? `Complete all ${weeklyCapacity} periods for this class first.` : 'Publish this timetable'} disabled={busy || selectedVersion?.status !== 'draft' || weeklyCapacity === 0 || entries.length !== weeklyCapacity} onClick={() => void run(async () => { await publishTimetableVersion(versionId); }, 'Timetable published.')} className="inline-flex h-9 items-center gap-2 rounded-lg bg-emerald-700 px-4 text-xs font-medium text-white disabled:opacity-40"><Rocket size={14} /> Publish timetable</button>}
      </div>
    </nav>
